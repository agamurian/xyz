(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.PanoramaComponent = factory());
    ...
    var vertex = "#define GLSLIFY 1\nattribute vec2 uv;attribute vec3 position;attribute vec3 normal;uniform mat4 modelViewMatrix;uniform mat4 projectionMatrix;varying vec2 vUv;void main(){vUv=vec2(1.0-uv.s,uv.t);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}"; // eslint-disable-line

    var fragment = "#ifdef GL_FRAGMENT_PRECISION_HIGH\nprecision highp float;precision highp int;\n#else\nprecision mediump float;precision mediump int;\n#define GLSLIFY 1\n#endif\nuniform sampler2D tMap;varying vec2 vUv;void main(){vec3 tex=texture2D(tMap,vUv).rgb;gl_FragColor.rgb=tex;gl_FragColor.a=1.0;}"; // eslint-disable-line
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	function makeScene() {
    		return new Mesh(gl,
    		{
    				geometry: new Sphere(gl, { radius: 2, widthSegments: 64 }),
    				program: new Program(gl,
    				{
    						cullFace: gl.FRONT,
    						uniforms: {
    							tMap: { value: TextureLoader.load(gl, { src }) }
    						},
    						vertex,
    						fragment
    					})
    	...
    	onMount(() => {
    		$$invalidate(8, renderer = new Renderer({
    				canvas,
    				width: wrapper.clientWidth,
    				height: wrapper.clientHeight
    			}));

    		$$invalidate(9, gl = renderer.gl);
    	...
    	$$self.$$set = $$props => {
    		...
    		if ("fov" in $$props) $$invalidate(6, fov = $$props.fov);
    		if ("src" in $$props) $$invalidate(7, src = $$props.src);
    	};

    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*clientWidth, clientHeight*/ 12) {
    			$$invalidate(10, aspect = clientWidth / clientHeight);
    		}

    		if ($$self.$$.dirty & /*gl, wrapper*/ 528) {
    			$$invalidate(11, camera = gl && wrapper && makeCamera());
    		}

    		if ($$self.$$.dirty & /*camera, canvas*/ 2080) {
    			controls = camera && canvas && makeControls();
    		}

    		if ($$self.$$.dirty & /*src, gl*/ 640) {
    			scene = src && gl && makeScene();
    		}

    		if ($$self.$$.dirty & /*renderer, aspect, clientWidth, clientHeight*/ 1292) {
    			if (renderer && aspect) {
    				renderer.setSize(clientWidth, clientHeight);
    			}
    		}

    		if ($$self.$$.dirty & /*camera, aspect*/ 3072) {
    			if (camera && aspect) {
    ...
    customElements.define("svelte-panorama", Component);
